...
embeddings=gensim.models.KeyedVectors.load_word2vec_format("twitter_sgns_subset.txt.gz" , binary=False) 
...
## NOW LOAD TEST DATA ##
TSdata = [ (row["Tweetid"], row["Tweet"] , float( row["Rating"] )  ) for row in csv.DictReader(open("anger_test.txt"), delimiter='\t', quoting=csv.QUOTE_NONE) ]
test_size = int(len(TSdata) )
test_texts = [ txt for ( tid, txt, label ) in TSdata[0:test_size] ]
test_labels = [ label for ( tid, txt , label ) in TSdata[0:test_size] ]
...
TSdata = [ ( row["Tweet"] , float( row["Rating"] )  ) for row in csv.DictReader(open("cnn_lstm/test/sadness_train.txt"), delimiter='\t', quoting=csv.QUOTE_NONE) ]
test_size = int(len(TSdata) )
...
for i in EMOS.split():
	...
	model.add(GRU(embeddings_dim))
	model.add(Dense(reg_dimensions))
	model.add(Activation('sigmoid'))
	model.compile(loss='mean_absolute_error', optimizer='adam')
        #train_sequences = np.array(train_sequences)
	train_labels = np.array(train_labels)
	#data_set = tf.data.Dataset.from_tensor_slices(  (train_sequences ,train_labels) )
	model.fit(train_sequences ,train_labels , batch_size=16,epochs = 10)
	model.save("cnn_lstm/models/"+currentemo+".h5")  # creates a HDF5 file 'my_model.h5'
	'''
	model = keras.models.load_model("cnn_lstm/models/"+currentemo+".h5")
	results = model.predict( test_sequences )
	np.savetxt("cnn_lstm/test/pred/"+currentemo+"3.txt", results, newline='\n')
